...
const getters = {
  ...
  layerFromModuleName: state => ({ moduleName }) => {
    const layerIndex = state.layers.findIndex(
      layer => layer.moduleOrder.indexOf(moduleName) > -1
    )
...
const actions = {
  ...
  removeFocusedLayer({ commit, state }) {
    ...
    Layer.moduleOrder.forEach(moduleName => {
      store.dispatch('modVModules/removeActiveModule', { moduleName })
    })
    commit('removeLayer', { layerIndex: state.focusedLayer })
    if (state.focusedLayer > 0)
      commit('setLayerFocus', { layerIndex: state.focusedLayer - 1 })
  },
  ...
  async removeAllLayers({ commit, state }) {
    const layerPromises = state.layers.map(async Layer => {
      const modulePromises = Layer.moduleOrder.map(moduleName =>
        store.dispatch('modVModules/removeActiveModule', {
          moduleName,
