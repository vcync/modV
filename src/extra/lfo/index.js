...
const lfoplugin = {
  ...
  install() {
    store.subscribe(mutation => {
      if (mutation.type === 'modVModules/removeActiveModule') {
        store.commit('lfo/removeAssignments', {
          moduleName: mutation.payload.moduleName
        })
      }

      if (mutation.type === 'tempo/setBpm') {
        store.dispatch('lfo/updateBpmFrequency', {
          frequency: hzFromBpm(mutation.payload.bpm)
        })
      }
    })
  ...
  makeValue({ currentValue, moduleName, controlVariable }) { //eslint-disable-line
    ...
    const control =
      store.state.modVModules.active[moduleName].props[controlVariable]
