...
class ModV {
  ...
  start(Vue) {
    ...
    mediaStreamScan()
      .then(mediaStreamDevices => {
        mediaStreamDevices.audio.forEach(source =>
          store.commit('mediaStream/addAudioSource', { source })
        )
        mediaStreamDevices.video.forEach(source =>
          store.commit('mediaStream/addVideoSource', { source })
        )
      ...
      .then(setMediaStreamSource)
      .then(({ audioSourceId, videoSourceId }) => {
...
export default modV
export { modV, Layer, webgl, isf, draw }
