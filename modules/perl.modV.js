var perl = function() {
	this.info = {
		...
		controls: [
			...
			{type: 'range', variable: 'intensity', label: 'ZCR Intensity', min: 0, max: 30, varType: 'int', step: 1},
	...
	this.init = function(canvas) {
		FIELDWIDTH = canvas.width, FIELDHEIGHT = canvas.height;
		randomize();
	};
	...
	function Particle(id) {
	  var id, x, y, xp, yp, s, d, sColor, len, z, zp;
		...
		this.update = function(ctx, meyda) {
			...
			d = (noise.noise(id, x/(that.mouseY + (meyda['zcr']/10 * that.intensity)), y/(that.mouseY + (meyda['zcr']/10 * that.intensity)))  -0.5) * that.mouseX;
